-- https://github.com/kism/mpvscripts/blob/1be4ac60f5c0eb40722142e56de907133acc32ef/scripts/vlccrop.lua
-- Added a crop override/reset function "manual_reset" with a keybinding "reset_crop"
...
function on_press()
	...
	mp.command(
		string.format(
			"%s vf pre @%s:lavfi-crop=w=%s:h=%s:x=%s:y=%s",
			command_prefix,
			cropstring,
			new_w,
			new_h,
...
function manual_reset()
	cleanup()
	crop_option = 0
	mp.osd_message("Crop: Original")
	mp.msg.info("Crop reset to original")
end

mp.add_key_binding("c", "toggle_crop", on_press)
mp.register_event("file-loaded", on_start)
mp.add_key_binding(nil, "reset_crop", manual_reset)
